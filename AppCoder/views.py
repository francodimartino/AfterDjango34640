...
def cursoFormulario(request):
    ...
    if request.method=="POST":
        form=CursoForm(request.POST)#viene form lleno con datos
        print("-------------------------------")
        print(form)
        print("-------------------------------")
        if form.is_valid():
            informacion=form.cleaned_data#convierto el form en un diccionario
            print(informacion)
            nombrecito=informacion["nombre"]
            comisioncita=informacion["comision"]
            fechita=informacion["fechaInicio"] #creo las variables con los datos del form

            curso1=Curso(nombre=nombrecito,comision=comisioncita, fechaInicio=fechita)#creo el curso
            curso1.save()#guardo el curso
            return render (request, "AppCoder/inicio.html")#en este caso si salio todo bien, voy a inicio (no es la mejor opcion)
        else:
